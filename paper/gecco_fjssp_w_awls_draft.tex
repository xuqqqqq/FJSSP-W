...
Our starting point is the adaptive-weighting local search (AWLS) philosophy of Zhang et al.~\cite{zhang2024awls}: a strong single-solution trajectory search can be highly competitive for FJSSP when tabu memory, landscape smoothing, and carefully screened neighborhood evaluation are combined.
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    ...
    \State $x^\star \gets x$; initialize tabu list $T \gets \emptyset$
    \While{budget $B$ is not exhausted}
      ...
      \ForAll{sampled moves $a$}
        ...
        \ElsIf{$a$ is tabu and does not satisfy aspiration}
          \State discard $a$
        \Else
          \State score $a$ by approximate local impact on duration, machine queues, and worker queues
        \EndIf
      \EndFor
      ...
      \Else
        \State $x \gets y$ and insert the reverse move attributes into $T$
...
First, this draft reports a competition-engineering workflow rather than a controlled ablation study; the marginal contribution of each neighborhood and screening component has not yet been isolated.
...
Future work should integrate explicit adaptive operation weights, run a full statistical evaluation, and report ablations for worker arcs, assignment moves, approximate screening, and warm-start promotion.

...
\title{Worker-Aware Critical-Path Tabu Search for Flexible Job Shop Scheduling with Worker Flexibility}
...
\begin{abstract}
Classical flexible job shop scheduling (FJSSP) models machine flexibility but usually assumes that human operators are either unlimited or irrelevant.
That assumption is too weak for human-machine collaborative workshops, where an operation is feasible only after choosing a compatible machine-worker pair and where the processing time depends on the full operation-machine-worker triple.
FJSSP with worker flexibility (FJSSP-W), also studied as the dual-resource constrained FJSP (DRCFJSP), therefore introduces a third decision layer and a third family of resource-conflict arcs.
This paper studies how a strong trajectory local search for FJSSP can be upgraded into a dual-resource local search without hiding worker feasibility inside post-processing.
The proposed method uses a worker-aware disjunctive graph, critical structure, neighborhood, approximate screening layer, and exact validation gate.
Starting from high-quality feasible schedules, the solver alternates bounded warm-start intensification, approximate neighborhood screening, exact worker-aware validation, and submission-grade regeneration with ten distinct random seeds per instance.
...
These results suggest that carefully adapting critical-path local search to the worker dimension can be competitive even without a heavy population model.
\end{abstract}
...
\keywords{Flexible job shop scheduling, worker flexibility, dual-resource scheduling, tabu search, local search, warm start}
...
This abstraction is useful but incomplete for real human-machine collaborative workshops.
In such shops, a machine is not automatically usable: it must be paired with a worker who can operate it, and the duration of an operation can change with the selected worker.
Ignoring the worker dimension can therefore produce schedules that are optimistic on paper but infeasible or brittle on the shop floor.

FJSSP with worker flexibility (FJSSP-W), also called DRCFJSP in recent literature, extends each operation decision from a machine choice to an operation-machine-worker triple~\cite{gong2018workerflex,gong2020habc,li2025ceamcp,hutter2025benchmark}.
This extension is not just a larger assignment table.
It introduces a third decision variable, a third set of disjunctive resource conflicts, and new failure modes for local search.
A move that shortens an operation on the selected machine may overload a worker queue; a sequence change that is neutral on machines may still change the longest path through worker arcs.

Recent studies make the model richer by adding green indicators, worker learning, fatigue, distributed factories, or other production features.
The FJSSP-W competition setting keeps the core scientific test clean: dual-resource feasibility and makespan minimization.
It is therefore a good setting for asking whether the strongest ideas from FJSSP trajectory search can be made genuinely worker-aware rather than merely repaired after the fact.

This paper documents a pragmatic FJSSP-W competition workflow built around worker-aware critical-path tabu search, warm starts, and strict promotion gates.
The implementation is influenced by the single-solution local-search lineage of Zhang et al.~\cite{zhang2024awls}, but the contribution claimed here is the worker-aware upgrade: representation, graph, moves, screening, and validation all explicitly see worker resources.
...
\section{Method}
\subsection{From Machine-Only to Dual-Resource Local Search}
Classical FJSSP local search is designed around the idea that each operation is assigned to a machine and inserted into a position on that machine.
...
In the AWLS lineage, a distinctive component is adaptive weighting: each operation has a cumulative weight and an idle count, and the estimator treats the induced penalty as additional processing time.
...
Our solver therefore keeps the single-solution tabu-search structure, but generalizes the state, graph, neighborhoods, screening score, and validation step to the worker dimension.
